import { InLiveBroker } from "~/in-live/in-live-broker.ts";
import type { CloudConfig } from "#types/mod.ts";
import { InCloudServer } from "~/serve/cloud-server.ts";

import type { ExtensionOptions } from "~/serve/types.ts";
...
class InCloudRunner {
  ...
  #initBroker() {
    ...
    const broker = new InLiveBroker(parseInt(port));
  ...
  async #initMigrator() {
    ...
    inCloud.init();
    const { autoTypes, autoMigrate } = inCloud.getExtensionConfig("core");
