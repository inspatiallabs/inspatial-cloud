import { type PathHandler, RequestPathHandler } from "~/serve/path-handler.ts";
import type { ExceptionHandler } from "#types/serve-types.ts";
import type { LifecycleHandlerRunner } from "~/serve/request-lifecycle.ts";
...
export class ExtensionManager {
  ...
  registerExtension(extension: CloudExtension): void {
    ...
    for (const afterMigrate of extension.afterMigrate.account) {
      this.afterMigrate.account.push(afterMigrate);
    }
    for (const afterMigrate of extension.afterMigrate.global) {
      this.afterMigrate.global.push(afterMigrate);
    }
  }
  /**
