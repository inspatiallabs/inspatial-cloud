...
> {
   ...
   */
  get inRoot(): string {
    return joinPath(this.#cloudRoot, ".inspatial");
  }

  get filesPath(): string {
    return joinPath(this.#cloudRoot, ".inspatial", "files");
  }
  #config: CloudConfig;
  orm!: InSpatialORM;
  api: CloudAPI;
  ...
  constructor(appName: N, options?: {
    ...
    this.#cloudRoot = normalizePath(Deno.cwd(), { toDirname: true });
    Deno.env.set("IN_ROOT", this.inRoot);
    this.inLog = inLog;

    loadCloudConfigFile(this.#cloudRoot);
