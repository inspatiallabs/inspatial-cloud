...
import type { EntryType } from "#orm";
import type { ORMFieldType } from "#orm/types";

const generateModels = new CloudAction("generateModels", {
  ...
  async run({ app, params }) {
    const modelsPath = params.path || `${app.orm.generatedRoot}/flutter/models`;
